...
export default function AddSongsToPlaylists({ user_id, playlist_id, playlists, playlistname }) {
  ...
  function checkPlaylistSong_Spotify_id(input_song_id_name ) {
    ...
    if (playlistSongs.length > 0) {
     ...
     // console.log(filtered_playlist_song)
      const filtered_playlist_particular_song = filtered_playlist_song.filter(playlistsongpart =>{
       ...
       // console.log(`ID: ${playlistsongpart.spotify_song_name} is not equal to ${input_song_id_name}`);
        return ( playlistsongpart.spotify_song_name === input_song_id_name);
      ...
      if (filtered_playlist_particular_song.length > 0) {
        console.log("duplicate song exists, song name: ", input_song_id_name);
        return true;
      } else {
  ...
  function addPlaylistSongs(song_id, song_name,title, artist, img) {
    ...
    return axios.post(`http://localhost:8080/playlistsongs`, playlistSongObject)
      .then((response) => {
        ...
        setNewPlaylistSongs([newPlaylistSongsRes, ...newPlaylistSongs]);
      ...
      <div className="featured">
        ...
        <div className="featured-songs">
          {featured?.tracks?.map((features) => (
            <Featured title={features.name} artist={features.artists[0].name} img={features.album.images[0].url} key={features.uri} addMultiFunctionPlaylistSongs={addMultiFunctionPlaylistSongs} song_id={features.uri} song_name={features.name} playlists={playlists} newPlaylistSongs={newPlaylistSongs} playlist_id={playlist_id} user_id={user_id}/>
